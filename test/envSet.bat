@ECHO OFF
SET GWL_logLevelStr=FATAL
SET GWL_fn=./logfiles/EnvTest.log
SET GWL_isFile=false
SET GWL_isConsole=true
SET GWL_isColor=true
SET GWL_isEpoch=false
SET GWL_nYr=1
SET GWL_isShowMs=false
SET GWL_isLocalTz=false
SET GWL_isConsoleTs=true
SET GWL_isRollAtStartup=true
SET GWL_isRollBySize=false
SET GWL_maxLogSizeKb=0
SET GWL_maxNumRollingLogs=0
SET GWL_rollingLogPath=./rolledfiles/rolledfiles2

